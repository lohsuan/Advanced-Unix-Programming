...
bool solve_maze_recursive(maze_t *mz, int x, int y, int path_index) {
    ...
    if (x <= 0 || x >= mz->w - 1 || y <= 0 || y >= mz->h - 1 || mz->blk[y][x] != 0 || visited[y][x]) {
        // Out of bounds or hit a wall or already visited this cell
        return false;
    }
    ...
    if (solve_maze_recursive(mz, x + 1, y, path_index + 1)) return true;  // Right
    if (solve_maze_recursive(mz, x, y + 1, path_index + 1)) return true;  // Down
    if (solve_maze_recursive(mz, x - 1, y, path_index + 1)) return true;  // Left
    if (solve_maze_recursive(mz, x, y - 1, path_index + 1)) return true;  // Up
...
void solve_maze(maze_t *mz) {
    // Clear the path and visited table
    ...
    solve_maze_recursive(mz, mz->sx, mz->sy, 0);
}
...
int calculate_direction_from_path(maze_t *mz, char *path_str) {
    int length = 0;
    for (int i = 0; i < _MAZE_MAXY * _MAZE_MAXX; i++) {
        if (path[i][0] == -1 || path[i][1] == -1) {
            break;
        }
        if (path[i][0] == path[i + 1][0] && path[i][1] == path[i + 1][1] + 1) {
            // printf("u ");
            path_str[i] = 'U';
        } else if (path[i][0] == path[i + 1][0] && path[i][1] == path[i + 1][1] - 1) {
            // printf("d ");
            path_str[i] = 'D';
        } else if (path[i][0] == path[i + 1][0] + 1 && path[i][1] == path[i + 1][1]) {
            // printf("l ");
            path_str[i] = 'L';
        } else if (path[i][0] == path[i + 1][0] - 1 && path[i][1] == path[i + 1][1]) {
            // printf("r ");
...
void print_dir(char *dir, int length) {
...
void *getSymbolAddrWithFuncName(char *maze_func_name) {
    ...
    void *result_addr = dlsym(handle, maze_func_name);
    // fprintf(stderr, "%s=%p\n", maze_func_name, result_addr);
...
int maze_init() {
    ...
    solve_maze(mz);
    char path_str[_MAZE_MAXY * _MAZE_MAXX];
    int length = calculate_direction_from_path(mz, path_str);
    printf("\nlength = %d\n", length);

    print_path();

    void *main_real_addr = maze_get_ptr();
    printf("main_real_addr = %p\n", main_real_addr);
    void *main_offset_addr = (void *)0x1b7a9;
    // void *main_offset_addr = (void *)0x16c69;  // server

    void *move_up_addr = getSymbolAddrWithFuncName("move_up");
    void *move_down_addr = getSymbolAddrWithFuncName("move_down");
    void *move_left_addr = getSymbolAddrWithFuncName("move_left");
    void *move_right_addr = getSymbolAddrWithFuncName("move_right");

    long page_size = sysconf(_SC_PAGE_SIZE);  // 4096 = 0x1000

    for (int i = 0; i < length; i++) {
        void *inject_addr = (void *)main_real_addr - (void *)main_offset_addr + (void *)hex_got_offset_table[i];

        ////// add write privilege
        long page_start_aligned = (long)inject_addr & ~(page_size - 1);
        // printf("page_start_aligned = %p\n", page_start_aligned);
        ...
        if (path_str[i] == 'U') {
            memcpy(inject_addr, &move_up_addr, sizeof(inject_addr));
        } else if (path_str[i] == 'D') {
            memcpy(inject_addr, &move_down_addr, sizeof(inject_addr));
        } else if (path_str[i] == 'L') {
            memcpy(inject_addr, &move_left_addr, sizeof(inject_addr));
        } else if (path_str[i] == 'R') {
            memcpy(inject_addr, &move_right_addr, sizeof(inject_addr));
        }
    }
    printf("\n--- inject done ---\n\n");
